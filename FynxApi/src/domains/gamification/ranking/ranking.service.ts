...
export class RankingService {
  ...
  static async getContributionData(userId: number): Promise<Record<string, number>> {
    ...
    const transactions = await db.all(`
      ...
      GROUP BY day
    `, [userId, dateStr]) as ContributionRow[];
  ...
  static async calculateScore(userId: number): Promise<ScoreCalculation & { league: string }> {
    ...
    `, [userId, currentMonth]) as MonthlyFinancialRow;
    ...
    const userScoreData = await db.get('SELECT carry_over_score, league FROM user_scores WHERE user_id = ?', [userId]) as ScoreRow | undefined;
  ...
  static async getAchievements(userId: number): Promise<Achievement[]> {
    ...
    const txCount = await db.get('SELECT COUNT(*) as count FROM transactions WHERE user_id = ?', [userId]) as CountRow;
    achievements.push({
      id: 'first_tx',
  ...
  static async getBadges(userId: number): Promise<Badge[]> {
    ...
    return userBadges.map(b => ({
      ...
      unlockedAt: b.earned_at as string
    }));
  }
