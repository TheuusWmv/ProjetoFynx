...
// Para rodar: npm run dev (frontend) e npx mocha tests/cadastrar-transacao-entrada.test.js
...
describe('Fynx - Adicionar transação (E2E) [skip login]', function () {
  this.timeout(120000);
  let driver;
  let mockServer;
  let transactions = [];
  ...
  it('vai direto para /dashboard e adiciona uma transação preenchendo todos os campos', async () => {
    ...
    // Procura por botões típicos de tour: "Próximo", "Fechar", "Pular", "Skip", "Finalizar"
    ...
    try {
      ...
      await inputs[0].sendKeys('Entrada Teste Selenium');
      ...
      await driver.executeScript(function (el, value) {
        ...
        el.blur && el.blur();
      }, inputs[0], 'Entrada Teste Selenium');
    ...
    // 3) Tipo
    try {
      const entrada = await tryFind(
        dialog,
        [
          By.xpath(".//label[contains(., 'Entrada') ]"),
          By.xpath(".//div[contains(., 'Entrada') and @role='radio']"),
          By.xpath(".//span[contains(., 'Entrada')]")
        ],
        1000
      );
      if (entrada) {
        try { await entrada.click(); } catch (e) { await driver.executeScript('arguments[0].click()', entrada); }
        await driver.sleep(200);
      }
    } catch {}
    ...
    await driver.sleep(800);
    const created = await tryFind(driver, [By.xpath("//*[contains(., 'Entrada Teste Selenium')]")], 8000);
    if (!created) {
    ...
    expect(text).to.include('Entrada Teste Selenium');
  });
});
